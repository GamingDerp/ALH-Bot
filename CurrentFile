...
class DerpBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        while True:
            ...
            await asyncio.sleep(3600)  # Update the index every hour
...
async def info(ctx):
    ...
    e.add_field(
        ...
        value=f"<:AnarchyLinksHub:750895092507869214> [Anarchy Links Hub](https://discord.gg/xt9CkXQ)"
              ...
              f"\n:gear: [Add ALH-Bot](https://discord.com/api/oauth2/authorize?client_id=749364874815078523&permissions=0&scope=bot)"
    ...
    for channels in bot.index.values():
        for channel, servers in channels.items():
            if search_term in str(channel).lower():
                channels[[c for c in server.text_channels if c.name == channel][0].mention] = None
    if channels:
        choice = await bot.get_choice(ctx, ["Channels", "Discord Names"], ctx.author)
        if choice == "Channels":
            return await bot.display(channels, context=ctx)
    discords = {}
    for channels in bot.index.values():
        for channel, servers in channels.items():
...
async def eight_ball(ctx):
    ...
    choice = ["Yes", "No", "Obviously", "Wtf??", "I'm not sure..", "Maybe...?", "Stop asking.", "Find out for yourself smh"]
    await ctx.send(random.choice(choice))
